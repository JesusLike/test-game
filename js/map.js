class Map {
	constructor(tiles, tilesheet, charlist) {
		this.tiles = tiles;
	...
	spawn(ctx) {
		for (let i in this.charlist) {
			...
			this.chars[char.x][char.y] = char.id;
			char.render(ctx);
		}
	}
	...
	render(ctx) {
		let map = this;
		map.tilesheet.onload = function () {
			for (let i = 0; i < map.tiles.length; i++) {
				for (let j = 0; j < map.tiles[i].length; j++) {
					if (map.tiles[i][j] != 0) {
						let tile = map.getTile(i, j);
						ctx.drawImage(map.tilesheet, tile[1] * tileSrcWidth, tile[0] * tileSrcHeight, tileSrcWidth, tileSrcHeight,
